...
if __name__ == "__main__":
    ...
    qid_exec_sqls = json.load(open(os.path.join(output_dir, "exec.json"), "r"))
    qid_pred = {}
    for qid, exec_sqls in qid_exec_sqls.items():
        sqls = exec_sqls["sql_logs"]
        sqls = [item["sql"] for item in sqls]
        qid_pred[int(qid)] = sqls
    print(f"len(qid_pred): {len(qid_pred)}")

    eval_dir = f"eval_results/{args.dataset_name}/{args.method_name}/{args.model_name}"
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, "join.json")
    ...
    for qid, preds in qid_pred.items():
        ...
        sql_logs = []
        for sql_node in sql_nodes:
            if sql_node.warning_cnt == min_warning_cnt:
                sql_logs.append(
                    {
